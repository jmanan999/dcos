...
async def test_mock_enrich_classifies_grievance(http: AsyncClient) -> None:
    ...
    g = await _file_grievance(http, "There is a huge pothole on the road near my house causing accidents daily.")
...
async def test_enrich_updates_grievance_status(http: AsyncClient) -> None:
    ...
    g = await _file_grievance(http, "Garbage not collected for 3 days. Stray dogs menace near colony.")
    token = create_local_token(role="field_officer", department_id=str(uuid.uuid4()))
    await http.post(
        f"/api/v1/ai/enrich/{g['grievance_id']}",
        headers={"Authorization": f"Bearer {token}"},
...
async def test_cluster_detection_with_similar_embeddings() -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        await session.execute(text(
            "UPDATE grievances SET embedding = CAST(:emb AS vector) WHERE id = :id"
        ...
        # Cleanup
        await session.execute(text("DELETE FROM grievances WHERE id IN (:g1, :g2)"), {"g1": g1_id, "g2": g2_id})
        await session.commit()
