...
async def test_dept_queue_requires_officer_permission(http: AsyncClient) -> None:
    ...
    r = await http.get("/api/v1/workforce/dept-queue",
                       headers={"Authorization": f"Bearer {token}"})
...
async def test_claim_transitions_to_in_progress(http: AsyncClient) -> None:
    ...
    async with AsyncSessionLocal() as s:
        ...
        await s.execute(text("INSERT INTO users (id, name, role) VALUES (:id, 'Test Officer', 'field_officer') ON CONFLICT DO NOTHING"), {"id": uid})
        await s.execute(text("INSERT INTO officers (id, user_id, department_id) VALUES (:id, :uid, :dept) ON CONFLICT DO NOTHING"),
                        {"id": oid, "uid": uid, "dept": dept_id})
        await s.execute(text("UPDATE grievances SET assigned_officer_id = :oid, status = 'ASSIGNED' WHERE id = :gid"),
                        {"oid": oid, "gid": gid})
        await s.commit()

    # Officer claims it
    token = create_local_token(user_id=uid, role="field_officer", department_id=dept_id)
    r = await http.post(f"/api/v1/workforce/grievances/{gid}/claim",
                        headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 200
...
async def test_resolve_blocked_without_proof(http: AsyncClient) -> None:
    ...
    async with AsyncSessionLocal() as s:
        await s.execute(text("SELECT set_config('app.bypass_rls', 'true', true)"))
        await s.execute(text("UPDATE grievances SET status = 'IN_PROGRESS' WHERE id = :id"), {"id": gid})
        await s.commit()
...
async def test_proof_verification_empty(http: AsyncClient) -> None:
    gid, _ = await _create_enriched_grievance(http)
    token = create_local_token(role="field_officer", department_id=str(uuid.uuid4()))
    r = await http.get(f"/api/v1/workforce/grievances/{gid}/proof",
                       headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 200
    data = r.json()
...
async def test_get_notes_returns_list(http: AsyncClient) -> None:
    gid, _ = await _create_enriched_grievance(http)
    token = create_local_token(role="field_officer", department_id=str(uuid.uuid4()))
    r = await http.get(f"/api/v1/workforce/grievances/{gid}/notes",
                       headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 200
    assert isinstance(r.json(), list)
...
async def test_handoff_reroutes_to_new_dept(http: AsyncClient) -> None:
    ...
    r = await http.post(
        f"/api/v1/workforce/grievances/{gid}/notes",
        json={"note": "This is actually a DJB issue not MCD",
              "is_handoff": True, "handoff_dept_id": new_dept_id},
        headers={"Authorization": f"Bearer {token}"},
    )
    assert r.status_code == 200
